...
def main():
    ...
    labeled_dataset, unlabeled_dataset, test_dataset = get_data()
        ...
        for i,(inputs_x,labels) in tqdm(enumerate(labeled_trainloader)):
           ...
           # print('probs:',logits_u_w.detach()/args.T)#, max_probs[:20])
            ...
            if (args.local_rank == 0) & (i%50==0):
                print(f'loss: {loss.item()} loss_x: {Lx.item()}, loss u:{Lu.item()} lbls:{num_lbl} pos {max_prob.sum().item()}')
            #print(i, start, end)
