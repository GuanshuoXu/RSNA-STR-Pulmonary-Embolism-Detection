    ...
    def validate(val_size):
        ...
        for (x_u_w, x_u_s), val_labels in val_loader:
            ...
            #targets_x = targets_x.to(args.device)
                ...
                num_lbls=max_mask.sum().item() + min_mask.sum().item()
                gt[lbl_count:lbl_count+num_lbls] = val_labels[min_mask+max_mask].detach().numpy()
                preds[lbl_count:lbl_count+num_lbls] = pseudo_label[min_mask+max_mask].detach().numpy()
                preds[lbl_count:lbl_count+num_lbls] = (preds[lbl_count:lbl_count+num_lbls] >= threshold_max)
                ...
                lbl_count+=num_lbls#num_pe+num_no)
...
#validaion phase
        ...
        if args.local_rank == 0:
            ...
            writer.add_scalar('val_ep/5.labels', per_labels, ep)
            writer.add_scalar('val_ep/6.abs pe', det_pe, ep)
